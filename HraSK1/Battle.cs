    ...
    internal class Battle
    {
        ...
        public void Souboj(Player p, Bytost nepritel)
        {
            while (p.health > 0 && nepritel.health > 0)
            {
                ...
                Console.WriteLine("Co uděláš? (1) Kouzlit | (2) Vypít mana potion (+30 Many)");
                ...
                if (akce == "2")
                {
                    if (p.manaPotions > 0)
                    {
                        ...
                        p.manaPotions--;
                        Console.WriteLine($"Vypil jsi lektvar many tvoje mana: {p.mana}");
                        Thread.Sleep(1000);
                    }
                    else
                    {
                        Console.WriteLine("Došly ti lektvary!");
                        Thread.Sleep(1000);
                        continue;
                    }
                }
                else
                {
                    ...
                    while (true)
                    {
                        Console.Write("Zadej číslo kouzla: ");
                        string vstup = Console.ReadLine();

                        if (int.TryParse(vstup, out volba) && volba >= 0 && volba < p.mojeKouzla.Count)
                        {
                            break;
                        }

                        Console.WriteLine("Neplatná volba! Vyber číslo ze seznamu.");
                    }
